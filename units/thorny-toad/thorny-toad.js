...
(function () {
  ...
  var UNIT_ID = "thorny-toad";
  var SOUND_KEY = "thorny-toad.sound";
  var AMBIENT_VOL = 0.42;
  var AMBIENT_DUCK = 0.08;
  ...
  function wireSound() {
    ...
    var hello = document.getElementById("toad-hello-video");
    ...
    function paint() {
      ...
      snd.setAttribute(
        "aria-label",
        wanted
          ? "Sound on. Click to mute ambient."
          : "Sound is muted. Click for ambient sound."
      );
      if (label) label.textContent = wanted ? "Sound on" : "Muted";
    }

    function helloPlaying() {
      return !!(hello && !hello.paused && !hello.ended);
    }
    ...
    if (hello) {
      ["play", "playing"].forEach(function (ev) {
        hello.addEventListener(ev, function () {
          if (wanted) {
            ambient.volume = AMBIENT_DUCK;
            var p = ambient.play();
            if (p && p.catch) p.catch(function () {});
